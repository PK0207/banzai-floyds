...
def fit_profile(data, uncertainty, wavelengths, orders, wavelength_bins, profile_width=4):
    ...
    for order, order_wavlengths in zip(orders, wavelength_bins):
        ...
        for wavelength_bin in order_wavlengths:
            ...
            wavelength_inds = np.logical_and(wavelengths[in_order] <= (wavelength_bin['center'] + wavelength_bin['width'] / 2.0), 
                                             wavelengths[in_order] >= (wavelength_bin['center'] - wavelength_bin['width'] / 2.0))
            ...
            best_fit_center, _ = maximize_match_filter((y_to_fit[np.argmax(data_to_fit)], 0.05), data_to_fit, error_to_fit, 
                                                                profile_gauss_fixed_width, y_to_fit, args=(fwhm_to_sigma(profile_width)))
...
def fit_background(data, uncertainty, wavelengths, profile_fits, orders, wavelength_bins, poly_order=4, default_width=4):
    # In principle, this should be some big 2d fit where we fit the profile center, the profile width, and the background in one go
    x2d, y2d = np.meshgrid(np.arange(data.shape[1])), np.arange(data.shape[0])
    ...
    for order, order_wavlengths, profile in zip(orders, wavelength_bins, profile_fits):
        ...
        for wavelength_bin in order_wavlengths:
            ...
            best_fit_sigma, *best_fit_coeffs = maximize_match_filter(initial_guess, data_to_fit, error_to_fit, background_fixed_profile_center, 
                                                                     y_to_fit, args=(profile(wavelength_bin['center'])))
            # If the peak of the profile is 2 > than the peak fo the background, keep the profile width
            if data_to_fit[peak] / error_to_fit[peak] > 2.0 * np.median(np.abs(data_to_fit / error_to_fit)):
            ...
            normalization /= np.sum(background_fixed_profile_center((best_fit_sigma, *best_fit_coeffs), 
                                                                     y_to_fit, profile(wavelength_bin['center'])) / error_to_fit / error_to_fit)
...
def get_wavelength_bins(wavelengths):
    ...
    all_bin_edges = [model(np.arange(min(model.domain) - 0.5, max(model.domain) + 1)) for model in wavelengths._polynomials]
    return [Table({'center': (bin_edges[1:] + bin_edges[:-1]) / 2.0 , 'width': bin_edges[1:] - bin_edges[:-1]}) for bin_edges in all_bin_edges]
...
def combine_wavelegnth_bins(wavelength_bins):
    ...
    wavelength_regions = [(min(order_bins['center']), max(order_bin['center'])) for order_bins in wavelength_bins]
    ...
    overlap_end_index = np.min(np.argwhere(wavelength_bins[red_order_index]['center'] > max(wavelength_regions[blue_order_index]['center'])))
...
class Extractor(Stage):
    def do_stage(self, image):
        ...
        profile_centers = fit_profile(image.data, image.uncertainty, image.wavelegnths, image.orders, image.wavelength_bins)
        background, profile_widths = fit_background(image.data, image.uncertainty, image.orders, profile_centers)
