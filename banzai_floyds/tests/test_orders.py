...
def test_blind_center_search():
    ...
    for center in order_centers:
        order_slice = order_region(order_width, Legendre([center], domain=(0, data.shape[1] - 1)),
                                   data.shape)
        data[order_slice] = 1000.0
        error[order_slice] = 100.0

    found_centers = estimate_order_centers(data, error, order_width)
...
def test_fit_orders():
    ...
    order_width = 85
    input_order_region = order_region(order_width, Legendre(input_center_params, domain=(0, data.shape[1] - 1)),
                                      data.shape)
    ...
    fitted_order_model = fit_order_curve(data, error, order_width, [order_center, 0, 0])
    fitted_order_region = order_region(order_width, fitted_order_model, data.shape)
...
def test_order_solver_stage():
    ...
    input_order_regions = [order_region(order_width, Legendre(params, domain=(0, data.shape[1] - 1)),
    ...
    order_solver.ORDER_WIDTH = order_width
